...
volatile unsigned long lastEspNowFrameRequest = 0; // Timestamp for priority locking (CrowPanel heartbeat)
extern uint8_t targetMac[6];
volatile bool isWebStreaming = false; // Flag to track active web stream
...
extern uint8_t flashBrightness;
...
<style>
    ...
    .btn-flash { background: #f59e0b; color: #000; box-shadow: 0 4px 15px rgba(245, 158, 11, 0.3); }
    ...
    <div class="container">
        ...
        <div class="card">
            <div class="label">STREAM OPTIMIZATION</div>
            <button class="btn btn-dir" style="background: #6366f1; width: 80%;" onclick="cycleQuality()">TOGGLE SPEED/QUALITY</button>
            <div id="qMsg" style="font-size: 0.8em; color: #94a3b8; margin-top: 10px;">Mode: High Detail</div>
        </div>

        <div class="card">
            <div class="label">FLASH BRIGHTNESS</div>
            <input type="range" min="0" max="255" value="0" class="slider" id="flashSlider" oninput="updateFlash(this.value)" style="accent-color: #f59e0b;">
        </div>
    ...
    <script>
        ...
        function updateFlash(val) { sendCmd('flash', val); }
        let qIdx = 0; const qVals = [10, 25, 50]; const qLabels = ["High Detail", "Balanced", "Turbo (Fastest)"];
        function cycleQuality() { qIdx = (qIdx + 1) % 3; document.getElementById('qMsg').innerText = "Mode: " + qLabels[qIdx]; sendCmd('quality', qVals[qIdx]); }
        function sendCmd(cmd, val) { fetch(`/control?cmd=${cmd}&val=${val}`); }
        ...
        setInterval(() => {
            ...
            fetch('/telemetry').then(r => r.json()).then(d => {
                ...
                if (document.activeElement.id !== 'flashSlider') {
                    document.getElementById('flashSlider').value = d.flash;
                }
...
void handleTelemetry() {
    ...
    json += "\"flash\":" + String(flashBrightness) + ",";
...
void handleControl() {
    ...
    // Priority Lock: If CrowPanel is active, ignore web control to save bandwidth/CPU
    if (millis() - lastEspNowFrameRequest < 5000) {
        server.send(503, "text/plain", "LOCKED: ESP-NOW Priority Active");
        return;
    }
    ...
    if (cmd == "quality") {
        ...
        if (s) s->set_quality(s, val.toInt());
    }
    if (cmd == "flash") {
        if (CAM_PIN_FLASH != -1) {
            flashBrightness = val.toInt();
            ledcWrite(2, flashBrightness);
        }
    }
    if (cmd == "maxCurr") {
...
void mjpegStreamTask(void *pvParameters) {
    ...
    isWebStreaming = true; // Set priority flag for WiFi

    while (client->connected()) {
        // Priority Lock: Exit stream if ESP-NOW is requesting frames
        if (millis() - lastEspNowFrameRequest < 5000) {
            Serial.println("MJPEG: Stream terminated - Priority to ESP-NOW");
            break;
        }
        ...
        esp_camera_fb_return(fb);
        taskYIELD(); // Yield only to other tasks, don't force sleep
    }
    isWebStreaming = false; // Release WiFi priority
    client->stop();
    delete client;
    vTaskDelete(NULL);
}
...
void handleStream() {
    ...
    // Priority Lock: Prevent new web stream if CrowPanel is active
    if (millis() - lastEspNowFrameRequest < 5000) {
        server.send(503, "text/plain", "Priority: ESP-NOW Active");
        return;
    }
...
void onEspNowRecv(const uint8_t *mac, const uint8_t *data, int len) {
    ...
    switch (cmd) {
        case 0x01: switchResRequested = true; break;
        case 0x02: // Start/Heartbeat for Continuous Stream
            frameRequested = true; 
            lastEspNowFrameRequest = millis(); // Update priority timestamp
            break;
        case 0x03: zoomRequested = true; break;
        
        // Motor Power: 0x10 [state]
        case 0x10: 
            if (len > 1) {
                systemEnabled = (data[1] == 1);
                if (systemEnabled) { safetyTripped = false; lowBatteryTripped = false; calibrationNeeded = true; }
                else { baselineCurrent = 0.0; }
            }
            break;

        // Motor Direction: 0x11 [0=FWD, 1=REV]
        case 0x11:
            if (len > 1) forwardDirection = (data[1] == 0);
            break;

        // Motor Speed: 0x12 [0-255]
        case 0x12:
            if (len > 1) {
                webBaseSpeed = data[1];
                preferences.putUChar("motSpd", webBaseSpeed);
        ...
        // Flash Brightness: 0x13 [0-255]
        case 0x13:
            if (len > 1 && CAM_PIN_FLASH != -1) {
                flashBrightness = data[1];
                ledcWrite(2, flashBrightness);
            }
            break;

        // Reset Graft Counter: 0x14
        case 0x14:
            penetrationCount = 0;
            preferences.putUInt("pCnt", 0);
            break;

        // Calibrate Baseline: 0x15
        case 0x15:
            calibrationNeeded = true;
            break;

        // Thresholds: 0x20 [MaxCurr*10], 0x21 [MinVolt*10]
        case 0x20:
            if (len > 1) { maxCurrentThreshold = data[1] / 10.0f; preferences.putFloat("maxCurr", maxCurrentThreshold); }
            break;
        case 0x21:
            if (len > 1) { minVoltageThreshold = data[1] / 10.0f; preferences.putFloat("minVolt", minVoltageThreshold); }
            break;

        case 0x04: // Camera Brightness (requires 2 bytes)
            if (len == 2) {
                sensor_t * s = esp_camera_sensor_get();
...
void espNowStreamTask(void *pvParameters) {
    ...
    while (1) {
        ...
        // FASTEST STREAM LOGIC: If a request was heard in the last 2 seconds, 
        // treat it as an active "Push" session.
        bool pushModeActive = (millis() - lastEspNowFrameRequest < 2000);
        if (isWebStreaming && !pushModeActive) { frameRequested = false; }

        if (!cameraActive || (!frameRequested && !pushModeActive)) {
            vTaskDelay(pdMS_TO_TICKS(1)); 
            continue; 
        }
        ...
        while (offset < fb->len) {
            ...
            // CRITICAL: esp_now_send returns immediately if the radio buffer has space.
            // We check return status to ensure we aren't dropping packets at the driver level.
            esp_err_t result = esp_now_send(targetMac, packetData, chunkSize + 2);
            
            // Expert Tune: 40-50us is the "Sweet Spot" for ESP32-S3 RX buffers
            // Any faster and the S3's WiFi RX buffer will overflow.
...
void setupWeb() {
    ...
    // CRITICAL: Disable WiFi Power Management to stop the "lazy" connection
    // This keeps the radio on at all times for immediate ESP-NOW/WiFi response
    esp_wifi_set_ps(WIFI_PS_NONE);
    Serial.println("WiFi Power Management: DISABLED (Performance Mode)");
