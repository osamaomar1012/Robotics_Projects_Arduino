...
void loop() {
    ...
    // Execute every 50ms for high-precision penetration detection
    if (millis() - lastUpdate > 50) {
        ...
        if (calibrationNeeded) {
            ...
            if (motorSpeed != targetSpeed || targetSpeed == 0) {
                ...
                // Collect exactly 40 samples (2 seconds at 50ms intervals)
        ...
        if (systemEnabled && abs(motorSpeed) > 50 && !calibrationNeeded && baselineCurrent > 0.05) {
            ...
            if (batteryCurrent > dynamicSpikeThreshold && !isPenetrating) {
                ...
                triggerGraftFeedback(); // Show immediate visual confirmation on OLED
            }
        }
